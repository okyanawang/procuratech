    ...
    <div class="container">
        ...
        <div class="container">
            <form action="{{ route('admin.project.category.update', ['id' => $category->id]) }}" class="h-full px-0 md:px-14 mb-10"
                ...
                <div class="flex justify-center gap-5">
                    <!-- The button to open modal -->
                    <label for="modal-delete-category" class="btn btn-error mt-5 w-50 modal-button text-white"><i
                            class="fa-solid fa-trash"></i>&nbsp; Delete Category</label>
                    <!-- The button to open modal -->
                    <label for="modal-update-category" class="btn btn-primary mt-5 w-50 modal-button"><i
                            class="fa-regular fa-pen-to-square"></i>&nbsp; Update data</label>

                </div>

                <!-- Put this part before </body> tag -->
                <input type="checkbox" id="modal-update-category" class="modal-toggle" />
                <div class="modal modal-bottom sm:modal-middle lg:pl-80">
                    <div class="modal-box">
                        <h3 class="font-bold text-lg">Update data ?</h3>
            ...
            <input type="checkbox" id="modal-delete-category" class="modal-toggle" />
            <div class="modal modal-bottom sm:modal-middle lg:pl-80">
                <div class="modal-box">
                    <h3 class="font-bold text-lg">Delete data?</h3>
                    <p class="py-4">Are you sure you want to delete the category? It will also delete all their relation
                        to
                        category</p>
                    <form action="{{ route('admin.project.category.delete', ['id' => $category->id]) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        <div class="modal-action">
                            <label for="modal-delete-category" class="btn btn-info">cancel</label>
                            <input type="submit" class="btn btn-primary" value="Submit">
                        </div>
                    </form>
                </div>
        ...
        <div class="container">
            <form action="{{ route('admin.project.task.update', ['id' => $task->id]) }}" class="h-full px-0 md:px-14 mb-10" method="POST"
                ...
                <div class="flex justify-center gap-5">
                    <!-- The button to open modal -->
                    <label for="modal-delete-task" class="btn btn-error mt-5 w-50 modal-button text-white"><i
                            class="fa-solid fa-trash"></i>&nbsp; Delete Task</label>
                    <!-- The button to open modal -->
                    <label for="modal-update-task" class="btn btn-primary mt-5 w-50 modal-button"><i
                            class="fa-regular fa-pen-to-square"></i>&nbsp; Update data</label>

                </div>

                <!-- Put this part before </body> tag -->
                <input type="checkbox" id="modal-update-task" class="modal-toggle" />
                <div class="modal modal-bottom sm:modal-middle lg:pl-80">
                    <div class="modal-box">
                        <h3 class="font-bold text-lg">Update data ?</h3>
                        <p class="py-4">Are you sure you want to update the task data? Data changes can be done anytime
                        </p>
                        <div class="modal-action">
                            <label for="modal-update-task" class="btn">cancel</label>
                            <input type="submit" class="btn btn-primary" value="Submit">
                        </div>
                    </div>
                </div>
            </form>
            <input type="checkbox" id="modal-delete-task" class="modal-toggle" />
            <div class="modal modal-bottom sm:modal-middle lg:pl-80">
                <div class="modal-box">
                    <h3 class="font-bold text-lg">Delete data?</h3>
                    <p class="py-4">Are you sure you want to delete the task? It will also delete all their relation
                        to
                        workers and items</p>
                    <form action="{{ route('admin.project.task.delete', ['id' => $task->id]) }}" method="POST">
                        @method('DELETE')
                        @csrf
                        <div class="modal-action">
                            <label for="modal-delete-task" class="btn btn-info">cancel</label>
                            <input type="submit" class="btn btn-primary" value="Submit">
                        </div>
                    </form>
                </div>
